;; This file was generated by wasmate.py, which is a script that converts
;; from the "flat" text assembly syntax emitted by LLVM into the s-expression
;; syntax expected by the spec repository.
;;
;; Note: this is a hack. A real toolchain will eventually be needed.
;;

(module
  (import $foo0 "" "foo0" (param))
  (import $foo1 "" "foo1" (param))
  (import $foo2 "" "foo2" (param))
  (import $foo3 "" "foo3" (param))
  (import $foo4 "" "foo4" (param))
  (import $foo5 "" "foo5" (param))
  (export "bar32" $bar32)
  (func $bar32
    (param i32)
    (local i32 i32 i32 i32)
    (block $BB0_8
      (block $BB0_7
        (block $BB0_6
          (block $BB0_5
            (block $BB0_4
              (block $BB0_3
                (block $BB0_2
                  (set_local 2 (get_local 0))
                  (set_local 3 (i32.const 23))
                  (set_local 4 (i32.gt_u (get_local 2) (get_local 3)))
                  (br_if $BB0_8 (get_local 4))
                  (switch (get_local 2) $BB0_2 $BB0_2 $BB0_2 $BB0_2 $BB0_2 $BB0_2 $BB0_2 $BB0_2 $BB0_3 $BB0_3 $BB0_3 $BB0_3 $BB0_3 $BB0_3 $BB0_3 $BB0_3 $BB0_4 $BB0_4 $BB0_4 $BB0_4 $BB0_4 $BB0_4 $BB0_5 $BB0_6 $BB0_7)
                )
                (call_import $foo0)
                (br $BB0_8)
              )
              (call_import $foo1)
              (br $BB0_8)
            )
            (call_import $foo2)
            (br $BB0_8)
          )
          (call_import $foo3)
          (br $BB0_8)
        )
        (call_import $foo4)
        (br $BB0_8)
      )
      (call_import $foo5)
    )
    (return)
  )
  (export "bar64" $bar64)
  (func $bar64
    (param i64)
    (local i64 i32 i64 i32)
    (block $BB1_8
      (block $BB1_7
        (block $BB1_6
          (block $BB1_5
            (block $BB1_4
              (block $BB1_3
                (block $BB1_2
                  (set_local 1 (get_local 0))
                  (set_local 3 (i64.const 23))
                  (set_local 4 (i64.gt_u (get_local 1) (get_local 3)))
                  (br_if $BB1_8 (get_local 4))
                  (set_local 2 (i32.wrap/i64 (get_local 1)))
                  (switch (get_local 2) $BB1_2 $BB1_2 $BB1_2 $BB1_2 $BB1_2 $BB1_2 $BB1_2 $BB1_2 $BB1_3 $BB1_3 $BB1_3 $BB1_3 $BB1_3 $BB1_3 $BB1_3 $BB1_3 $BB1_4 $BB1_4 $BB1_4 $BB1_4 $BB1_4 $BB1_4 $BB1_5 $BB1_6 $BB1_7)
                )
                (call_import $foo0)
                (br $BB1_8)
              )
              (call_import $foo1)
              (br $BB1_8)
            )
            (call_import $foo2)
            (br $BB1_8)
          )
          (call_import $foo3)
          (br $BB1_8)
        )
        (call_import $foo4)
        (br $BB1_8)
      )
      (call_import $foo5)
    )
    (return)
  )
  (memory 0 0
    (segment 0
      ""
    )
  )
)
